...
from ..models import ExercicioCatalogo, Procedimento, Sessao
...
class SessionExerciseHistoryService:
    ...
    def __init__(self, procedimento: Procedimento):
        ...
        self.exercise_ids_by_session = {
            sessao.pk: self.get_selected_ids_for_session(sessao)[0]
            for sessao in self.sessions
        }

    def get_selected_ids_for_session(self, sessao: Sessao) -> tuple[set[int], str]:
        explicit_ids = {
            item.exercicio_id
            for item in sessao.sessao_exercicios.all()
            if item.is_active
        }
        if explicit_ids:
    ...
    def get_exercise_status_for_session(
        ...
        *,
        ...
        projected_consecutive_usage_count = (
            prior_consecutive_usage_count + 1 if used_in_previous_session else 1
        )
        if is_selected:
        ...
        if used_in_previous_session or consecutive_usage_count > 1:
            ...
            tooltip = (
                f"Exercício utilizado há {sessions_since_last_use} sessão(ões), "
                "ainda dentro do período recente."
            )
        else:
            state = STATUS_NORMAL
            label = "Disponível"
    ...
    def get_status_map_for_session(
        ...
        return {
            exercicio.pk: self.get_exercise_status_for_session(sessao, exercicio)
